...
class EqualByJsonMixin(object):
    ...
    def __eq__(self, that):
        try:
            ...
            return False
        return json.loads(str(self)) == that_obj
...
class CanonicalPropertiesMixin(PropertiesMixin):
    ...
    def canonical_properties(self):
        ...
        return collections.OrderedDict(
            (key, props[key]) for key in CANONICAL_FIELD_ORDER if key in props
        )


class Schema(CanonicalPropertiesMixin):
    """Base class for all Schema classes."""

    _reserved_properties = SCHEMA_RESERVED_PROPS

    def __init__(self, type_, other_props=None, validate_names=True):
        type_ = str(type_)
        if not isinstance(type_, str):
            raise avro.errors.SchemaParseException("Schema type must be a string.")
        if type_ not in VALID_TYPES:
            raise avro.errors.SchemaParseException("{type_} is not a valid type.")
...
class NamedSchema(Schema):
    ...
    def __init__(
        ...
        super(NamedSchema, self).__init__(
            type_, other_props, validate_names=validate_names
        )
        if not name:
        ...
        name = str(name)
        namespace = str(namespace) if namespace else namespace
    ...
    def __init__(self, precision, scale=0, max_precision=0):
        if not isinstance(precision, int) or precision <= 0:
            raise avro.errors.IgnoredLogicalType(
                "Invalid decimal precision {precision}. Must be a positive integer."
            )

        if precision > max_precision:
            raise avro.errors.IgnoredLogicalType(
                "Invalid decimal precision {precision}. Max is {max_precision}."
            )

        if not isinstance(scale, int) or scale < 0:
            raise avro.errors.IgnoredLogicalType(
                "Invalid decimal scale {scale}. Must be a non-negative integer."
            )

        if scale > precision:
            raise avro.errors.IgnoredLogicalType(
                "Invalid decimal scale {scale}. Cannot be greater than precision {precision}."
            )
...
class Field(CanonicalPropertiesMixin, EqualByJsonMixin):
    _reserved_properties = FIELD_RESERVED_PROPS
    ...
    def __init__(
        ...
        if order is not None and order not in VALID_FIELD_SORT_ORDERS:
            raise avro.errors.SchemaParseException(
                "The order property {order} is not valid."
            )
        self._has_default = has_default
        self.props.update(other_props or {})

        if not isinstance(type_, (dict, list)):
            type_ = str(type_)
        if isinstance(type_, str) and names is not None and names.has_name(type_, None):
            type_schema = names.get_name(type_, None)
        else:
            try:
                type_schema = make_avsc_object(
                    type_, names, validate_names=validate_names
                )
            except Exception as e:
                raise avro.errors.SchemaParseException(
                    'Type property "{type_}" not a valid Avro schema: {e}'
                )
...
class PrimitiveSchema(EqualByPropsMixin, Schema):
    ...
    def __init__(self, type, other_props=None):
        ...
        if type not in PRIMITIVE_TYPES:
            raise avro.errors.AvroException("{type} is not a valid primitive type.")
...
class BytesDecimalSchema(PrimitiveSchema, DecimalLogicalSchema):
    def __init__(self, precision, scale=0, other_props=None):
        DecimalLogicalSchema.__init__(
            self, precision, scale, max_precision=((1 << 31) - 1)
        )
...
class FixedSchema(EqualByPropsMixin, NamedSchema):
    def __init__(
        self, name, namespace, size, names=None, other_props=None, validate_names=True
    ):
        # Ensure valid ctor args
        if not isinstance(size, int) or size < 0:
            fail_msg = (
                "Fixed Schema requires a valid positive integer for size property."
            )
    ...
    def match(self, writer):
        ...
        @return bool
        """
        return self.type == writer.type and self.check_props(
            writer, ["fullname", "size"]
        )
...
class EnumSchema(EqualByPropsMixin, NamedSchema):
    def __init__(
        ...
        """
        @arg validate_enum_symbols, will allow enum symbols that are not valid Avro names and default, which is not an enumerated symbol.
        """
            ...
            for symbol in symbols:
                try:
                    ...
                    raise avro.errors.InvalidName(
                        "An enum symbol must be a valid schema name."
                    )

        if len(set(symbols)) < len(symbols):
            raise avro.errors.AvroException("Duplicate symbol: {symbols}")

        # Call parent ctor
        NamedSchema.__init__(
            self, "enum", name, namespace, names, other_props, validate_names
        )
        ...
        if validate_enum_symbols and other_props and "default" in other_props:
            ...
            if default not in symbols:
                raise avro.errors.InvalidDefault(
                    "Enum default '{default}' is not a valid member of symbols '{symbols}'"
                )

    @property
    def symbols(self):
        symbols = self.get_prop("symbols")
        if isinstance(symbols, (list, set, dict, tuple)):
            return symbols
        raise Exception
...
class ArraySchema(EqualByJsonMixin, Schema):
    def __init__(self, items, names=None, other_props=None, validate_names=True):
        # Call parent ctor
        ...
        if isinstance(items, str) and names.has_name(items, None):
            ...
            try:
                items_schema = make_avsc_object(
                    items, names, validate_names=self.validate_names
                )
            except avro.errors.SchemaParseException as e:
                fail_msg = "Items schema ({items}) not a valid Avro schema: {e} (known names: {names.names.keys()})"
    ...
    def match(self, writer):
        ...
        @return bool
        """
        return self.type == writer.type and self.items.check_props(
            writer.items, ["type"]
        )
...
class MapSchema(EqualByJsonMixin, Schema):
    def __init__(self, values, names=None, other_props=None, validate_names=True):
        # Call parent ctor
        ...
        if isinstance(values, str) and names.has_name(values, None):
            ...
            try:
                values_schema = make_avsc_object(
                    values, names, validate_names=self.validate_names
                )
            except avro.errors.SchemaParseException:
                raise
            except Exception:
                raise avro.errors.SchemaParseException(
                    "Values schema is not a valid Avro schema."
                )
    ...
    def match(self, writer):
        ...
        @return bool
        """
        return writer.type == self.type and self.values.check_props(
            writer.values, ["type"]
        )
    ...
    def validate(self, datum):
        ...
        return (
            self
            if isinstance(datum, dict) and all(isinstance(key, str) for key in datum)
            else None
        )
...
class UnionSchema(EqualByJsonMixin, Schema):
    ...
    def __init__(self, schemas, names=None, validate_names=True):
        # Ensure valid ctor args
        ...
        for schema in schemas:
            if isinstance(schema, str) and names.has_name(schema, None):
                ...
                try:
                    new_schema = make_avsc_object(
                        schema, names, validate_names=self.validate_names
                    )
                except Exception as e:
                    raise avro.errors.SchemaParseException(
                        "Union item must be a valid Avro schema: {e}"
                    )
            ...
            if (
                ...
                raise avro.errors.SchemaParseException(
                    "{new_schema.type} type already in Union"
                )
    ...
    def match(self, writer):
        ...
        @return bool
        """
        return writer.type in {"union", "error_union"} or any(
            s.match(writer) for s in self.schemas
        )
...
class RecordSchema(EqualByJsonMixin, NamedSchema):
    @staticmethod
    def make_field_objects(field_data, names, validate_names=True):
        """We're going to need to make message parameters too."""
        field_objects = []
        field_names = []
        for field in field_data:
            if not callable(getattr(field, "get", None)):
                raise avro.errors.SchemaParseException("Not a valid field: {field}")
            type = field.get("type")
            name = field.get("name")
            ...
            if new_field.name in field_names:
                fail_msg = "Field name {new_field.name} already in use."
                raise avro.errors.SchemaParseException(fail_msg)
    ...
    def match(self, writer):
        ...
        return writer.type == self.type and (
            self.type == "request" or self.check_props(writer, ["fullname"])
        )
    ...
    def __init__(
        ...
        if schema_type == "record":
            ...
            names.default_namespace = Name(
                name, namespace, names.default_namespace, validate_name=validate_names
            ).space

        # Add class members
        field_objects = RecordSchema.make_field_objects(
            fields, names, validate_names=validate_names
        )
    ...
    def to_canonical_json(self, names=None):
        ...
        if self.type == "request":
            raise NotImplementedError(
                "Canonical form (probably) does not make sense on type request"
            )
    ...
    def validate(self, datum):
        ...
        return (
            ...
            and {f.name for f in self.fields}.issuperset(datum.keys())
            else None
        )
...
class TimestampMillisSchema(LogicalSchema, PrimitiveSchema):
    ...
    def validate(self, datum):
        return (
            ...
            and _is_timezone_aware_datetime(datum)
            else None
        )
...
def make_bytes_decimal_schema(other_props):
    ...
    return BytesDecimalSchema(
        other_props.get("precision"), other_props.get("scale", 0), other_props
    )
...
def make_logical_schema(logical_type, type_, other_props):
    ...
    try:
        ...
        expected_types = sorted(
            literal_type for lt, literal_type in logical_types if lt == logical_type
        )
        if expected_types:
            warnings.warn(
                avro.errors.IgnoredLogicalType(
                    "Logical type {logical_type} requires literal type {'/'.join(expected_types)}, not {type_}."
                )
            )
        else:
            warnings.warn(
                avro.errors.IgnoredLogicalType("Unknown {logical_type}, using {type_}.")
            )
...
def make_avsc_object(
    json_data, names=None, validate_enum_symbols=True, validate_names=True
):
    """
    Build Avro Schema from data parsed out of JSON string.

    @arg names: A Names object (tracks seen names and default space)
    @arg validate_enum_symbols, will allow enum symbols that are not valid Avro names.
    """
    ...
    if callable(getattr(json_data, "get", None)):
        ...
        if type_ in NAMED_TYPES:
            ...
            name = str(name) if name is not None else name
            if not isinstance(name, str):
                raise avro.errors.SchemaParseException(
                    "Name {name} must be a string, but it is {t}.".format(
                        name=name, t=type(name)
                    )
                )
            ...
            if type_ == "fixed":
                ...
                if not isinstance(symbols, (list, set, dict, tuple)):
                    raise avro.errors.SchemaParseException(
                        "Enum symbols must be a sequence of strings, but it is {type(symbols)}"
                    )
                for symbol in symbols:
                    if not isinstance(symbol, str):
                        raise avro.errors.SchemaParseException(
                            "Enum symbols must be a sequence of strings, but one symbol is a {type(symbol)}"
                        )
            ...
            raise avro.errors.SchemaParseException("Unknown Named Type: {type_}")
        ...
        if type_ in VALID_TYPES:
            if type_ == "array":
                ...
                raise avro.errors.SchemaParseException("Unknown Valid Type: {type_}")
        elif type_ is None:
            raise avro.errors.SchemaParseException('No "type" property: {json_data}')
        else:
            raise avro.errors.SchemaParseException("Undefined type: {type_}")
    # JSON array (union)
    ...
    # not for us!
    fail_msg = "Could not make an Avro Schema object from {json_data}"
    raise avro.errors.SchemaParseException(fail_msg)


def parse(json_string, validate_enum_symbols=True, validate_names=True):
    """Constructs the Schema from the JSON text.

    @arg json_string: The json string of the schema to parse
    @arg validate_enum_symbols, will allow enum symbols that are not valid Avro names.
    @arg validate_names, will allow names that are not valid Avro names. When disabling the validation
    ...
    try:
        ...
        raise avro.errors.SchemaParseException(
            "Error parsing JSON: {json_string}, error = {e}"
        )  # from e
    return make_avsc_object(
        json_data,
        Names(validate_names=validate_names),
        validate_enum_symbols,
        validate_names,
    )


def from_path(path, validate_enum_symbols=True, validate_names=True):
    """
    Constructs the Schema from a path to an avsc (json) file.
    """
    with open(path, "r") as f:
        return parse(f.read(), validate_enum_symbols, validate_names)
