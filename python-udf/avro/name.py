...
def validate_basename(basename):
    ...
    if not _BASE_NAME_PATTERN.search(basename):
        raise avro.errors.InvalidName(
            str(basename)
            + " is not a valid Avro name because it does not match the pattern {_BASE_NAME_PATTERN.pattern!s}"
        )
...
class Name:
    ...
    def __init__(
        ...
        self._full = (
            ...
            else str(space_attr or default_space) + "." + str(name_attr)
        )
        if validate_name:
    ...
    def __eq__(self, other):
        ...
        return hasattr(other, "fullname") and self.fullname == getattr(
            other, "fullname"
        )
...
class Names:
    ...
    def __init__(self, default_namespace=None, validate_names=True):
        self.names = {}
        self.default_namespace = default_namespace
        self.validate_names = validate_names

    def has_name(self, name_attr, space_attr=None):
        test = Name(
            name_attr,
            space_attr,
            self.default_namespace,
            validate_name=self.validate_names,
        ).fullname
        return test in self.names

    def get_name(self, name_attr, space_attr=None):
        test = Name(
            name_attr,
            space_attr,
            self.default_namespace,
    ...
    def add_name(self, name_attr, space_attr, new_schema):
        ...
        if to_add.fullname in PRIMITIVE_TYPES:
            raise avro.errors.SchemaParseException(
                "{to_add.fullname} is a reserved type name."
            )
        if to_add.fullname in self.names:
            raise avro.errors.SchemaParseException(
                'The name "{to_add.fullname}" is already in use.'
            )
        if to_add.fullname is None:
            raise avro.errors.SchemaParseException(
                'The name built from "{space_attr}.{name_attr}" is None'
            )
