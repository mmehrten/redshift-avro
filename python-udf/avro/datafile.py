    ...
    json.dumps(
        {
            ...
            "fields": [
                {
                    "name": "magic",
                    "type": {"type": "fixed", "name": "magic", "size": MAGIC_SIZE},
                },
...
class _DataFileMetadata:
    ...
    def codec(self, value):
        ...
        if value not in VALID_CODECS:
            raise avro.errors.DataFileException("Unknown codec: {value!r}")
...
class DataFileWriter(_DataFileMetadata):
    ...
    def __init__(
        ...
        if hasattr(writer, "mode") and "b" not in writer.mode:
            warnings.warn(
                avro.errors.AvroWarning(
                    "Writing binary data to a writer {writer!r} that's opened for text"
                )
            )
...
class DataFileReader(_DataFileMetadata):
    ...
    def __init__(self, reader, datum_reader):
        if hasattr(reader, "mode") and "b" not in reader.mode:
            warnings.warn(
                avro.errors.AvroWarning(
                    "Reader binary data from a reader {reader!r} that's opened for text"
                )
            )
    ...
    def _read_header(self):
        ...
        if header.get("magic") != MAGIC:
            raise avro.errors.AvroException(
                "Not an Avro data file: {header.get('magic')!r} doesn't match {MAGIC!r}."
            )
    ...
    def next(self):
        ...
        if self.datum_decoder is None:
            raise avro.errors.DataFileException(
                "DataFile is not ready to read because it has no decoder"
            )
