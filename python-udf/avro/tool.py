...
def _parse_args():
    ...
    subparsers = (
        parser.add_subparsers(required=True, dest="command")
        if sys.version_info >= (3, 7)
        else parser.add_subparsers(dest="command")
    )
    ...
    subparser_rpcreceive.add_argument(
        "-file", type=argparse.FileType("rb"), required=False
    )
...
def main_dump(args: argparse.Namespace):
    print(
        "\n".join(
            f"{d!r}"
            for d in avro.datafile.DataFileReader(
                args.input_file, avro.io.DatumReader()
            )
        )
    )
    return 0
...
def main_rpcreceive(args: argparse.Namespace):
    datum = None
    if args.file:
...
def main_rpcsend(args: argparse.Namespace):
    datum = None
    if args.file:
        with avro.datafile.DataFileReader(args.file, avro.io.DatumReader()) as dfr:
            datum = next(dfr)
