...
STRUCT_FLOAT = struct.Struct("<")  # big-endian float
...
def validate(expected_schema, datum, raise_on_error=False):
    ...
    :param datum, A python dictionary or some supported type
    ...
    nodes = collections.deque(
        [ValidationNode(expected_schema, datum, getattr(expected_schema, "name", None))]
    )
    ...
    while nodes:
        ...
        valid_node = (
            ValidationNode(validated_schema, current_node.datum, current_node.name)
            if validated_schema
            else None
        )
        ...
        if valid_node is None:
            if raise_on_error:
                raise avro.errors.AvroTypeException(
                    current_node.schema, current_node.name, current_node.datum
                )
...
def _record_iterator(node):
    ...
    schema, datum, _ = node
    return (
        ValidationNode(field.type, datum.get(field.name), field.name)
...
def _map_iterator(node):
    ...
    return (
        ValidationNode(child_schema, child_datum, child_name)
        for child_name, child_datum in datum.items()
    )
...
class BinaryDecoder:
    ...
    def read(self, n):
        ...
        if n < 0:
            raise avro.errors.InvalidAvroBinaryEncoding(
                "Requested {n} bytes to read, expected positive integer."
            )
        read_bytes = self.reader.read(n)
        if len(read_bytes) != n:
            raise avro.errors.InvalidAvroBinaryEncoding(
                "Read {len(read_bytes)} bytes, expected {n} bytes"
            )
        return read_bytes
    ...
    def read_timestamp_millis_from_long(self):
        ...
        unix_epoch_datetime = datetime.datetime(
            1970, 1, 1, 0, 0, 0, 0, tzinfo=avro.timezones.utc
        )
...
class BinaryEncoder:
    ...
    def write_bytes(self, datum):
        ...
        self.write(struct.pack("{len(datum)}s", datum))
    ...
    def write_time_micros_long(self, datum):
        ...
        """
        microseconds = (
            datum.hour * 3600000000
            + datum.minute * 60000000
            + datum.second * 1000000
            + datum.microsecond
        )
        self.write_long(microseconds)

    def _timedelta_total_microseconds(self, timedelta_delta):
        return (
            timedelta_.microseconds
            + (timedelta_.seconds + timedelta_.days * 24 * 3600) * 10 ** 6
        )

    def write_timestamp_millis_long(self, datumtime):
        """
...
class DatumReader:
    ...
    def writers_schema(self, writers_schema):
        self._writers_schema = writers_schema

    @property
    def readers_schema(self):
        return self._readers_schema

    @readers_schema.setter
    def readers_schema(self, readers_schema):
        self._readers_schema = readers_schema

    def read(self, decoder):
        if self.writers_schema is None:
            raise avro.errors.IONotReadyException(
                "Cannot read without a writer's schema."
            )
    ...
    def read_data(self, writers_schema, readers_schema, decoder):
        ...
        if isinstance(writers_schema, avro.schema.UnionSchema) and isinstance(
        ...
        if writers_schema.type == "bytes":
            ...
            return decoder.read_bytes()
        if isinstance(writers_schema, avro.schema.FixedSchema) and isinstance(
            readers_schema, avro.schema.FixedSchema
        ):
            if logical_type == "decimal":
                precision = writers_schema.get_prop("precision")
                if not (isinstance(precision, int) and precision > 0):
                ...
                return decoder.read_decimal_from_fixed(
                    precision, scale, writers_schema.size
                )
            return self.read_fixed(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, avro.schema.EnumSchema) and isinstance(
            readers_schema, avro.schema.EnumSchema
        ):
            return self.read_enum(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, avro.schema.ArraySchema) and isinstance(
            readers_schema, avro.schema.ArraySchema
        ):
            return self.read_array(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, avro.schema.MapSchema) and isinstance(
            readers_schema, avro.schema.MapSchema
        ):
            return self.read_map(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, avro.schema.RecordSchema) and isinstance(
            readers_schema, avro.schema.RecordSchema
        ):
            # .type in ["record", "error", "request"]:
            return self.read_record(writers_schema, readers_schema, decoder)
        raise avro.errors.AvroException(
            "Cannot read unknown schema type: {writers_schema.type}"
        )
    ...
    def skip_data(self, writers_schema, decoder):
        ...
        raise avro.errors.AvroException("Unknown schema type: {writers_schema.type}")
    ...
    def read_enum(
        ...
        if index_of_symbol >= len(writers_schema.symbols):
            raise avro.errors.SchemaResolutionException(
                "Can't access enum index {index_of_symbol} for enum with {len(writers_schema.symbols)} symbols",
                writers_schema,
                readers_schema,
            )
        ...
        if read_symbol not in readers_schema.symbols:
            raise avro.errors.SchemaResolutionException(
                "Symbol {read_symbol} not present in Reader's Schema",
                writers_schema,
                readers_schema,
            )
    ...
    def read_array(
        ...
        while block_count != 0:
            ...
            for i in range(block_count):
                read_items.append(
                    self.read_data(writers_schema.items, readers_schema.items, decoder)
                )
            block_count = decoder.read_long()
        return read_items
    ...
    def read_map(
        ...
        while block_count != 0:
            ...
            for i in range(block_count):
                ...
                read_items[key] = self.read_data(
                    writers_schema.values, readers_schema.values, decoder
                )
            block_count = decoder.read_long()
        return read_items
    ...
    def read_union(
        ...
        if index_of_schema >= len(writers_schema.schemas):
            raise avro.errors.SchemaResolutionException(
                "Can't access branch index {index_of_schema} for union with {len(writers_schema.schemas)} branches",
                writers_schema,
                readers_schema,
            )
            ...
            for field_name, field in readers_fields_dict.items():
                if field_name not in writers_fields_dict:
                    if not field.has_default:
                        raise avro.errors.SchemaResolutionException(
                            "No default value for field {field_name}",
                            writers_schema,
                            readers_schema,
                        )
    ...
    def _read_default_value(self, field_schema, default_value):
        ...
        if isinstance(field_schema, avro.schema.ArraySchema):
            if isinstance(default_value, Iterable):
                return [
                    self._read_default_value(field_schema.items, json_val)
        ...
        if isinstance(field_schema, avro.schema.MapSchema):
            if isinstance(default_value, Mapping):
                return {
                    key: self._read_default_value(field_schema.values, json_val)
        ...
        if isinstance(field_schema, avro.schema.RecordSchema):
            ...
            for field in field_schema.fields:
                ...
                read_record[field.name] = field_val
            return read_record
        raise avro.errors.AvroException("Unknown type: {field_schema.type}")
...
class DatumWriter:
    ...
    def write(self, datum, encoder):
        if self.writers_schema is None:
            raise avro.errors.IONotReadyException(
                "Cannot write without a writer's schema."
            )
    ...
    def write_data(self, writers_schema, datum, encoder):
        ...
        if writers_schema.type == "long":
            if logical_type == avro.constants.TIME_MICROS:
                ...
                warnings.warn(
                    avro.errors.IgnoredLogicalType("{datum} is not a datetime type")
                )
        ...
        if writers_schema.type == "bytes":
            if logical_type == "decimal":
                ...
                if not (isinstance(scale, int) and scale >= 0):
                    ...
                    warnings.warn(
                        avro.errors.IgnoredLogicalType("{datum} is not a decimal type")
                    )
                else:
        ...
        if isinstance(writers_schema, avro.schema.RecordSchema):
            ...
            raise avro.errors.AvroTypeException(writers_schema, datum)
        raise avro.errors.AvroException("Unknown type: {writers_schema.type}")
