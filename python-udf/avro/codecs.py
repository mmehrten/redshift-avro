...
def _check_crc32(bytes_, checksum):
...
class Codec(abc.ABCMeta):
    ...
    @staticmethod
    @abc.abstractmethod
    def decompress(readers_decoder):
...
def get_codec(codec_name):
    try:
        ...
        raise avro.errors.UnsupportedCodec(
            "Unsupported codec: {codec_name}. (Is it installed?)"
        )
